...
# Extract data directly from the boxes
detection_ids = []
bounding_boxes = {}
...
with open("zone_transitions.txt", "w") as zones_file, \
    ...
    while True:
        ...
        for idx, box in enumerate(boxes):
            ...
            bounding_boxes[idx] = ([xmin, ymin, xmax, ymax, confidence])
        
        last_ids = detection_ids
        last_boxes = bounding_boxes
        for key, value in bounding_boxes.items():
            for key2, value2 in last_boxes.items():
                if value2[0]-value[0]<10 and value2[1]-value[1]<10:
                    bounding_boxes[value] = key2
        ...
        for idx, box in enumerate(boxes):
            ...
            if(index_frame%20==0):
                detection.append([xmin, ymin, xmax, ymax, idx])
                centers[idx] = center_y
            if abs(centers[idx]-center_y)>50 :
                for detec in detection:
                    x1, y1, x2, y2, id = detec
                    if id==idx:
                    ...
                    if y_botella > height / 2:
                        if len(areas) >= 4:
                            if center_y > 0 and center_y <= areas['y1'].iloc[0]:
                                ...
                                current_zone = 2
                            elif (center_y > areas['y1'].iloc[1] +10 and center_y <= areas['y1'].iloc[2]) and xmax > x_tank:
                                current_zone = 3
