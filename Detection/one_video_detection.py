...
#video = cv.VideoCapture("D:/FutureExpertData/Computervision/GX016355.MP4")
video = cv.VideoCapture("D:\Chris\Lubinas\cortado_laberinto2.mp4")
...
with open("zone_transitions.txt", "w") as zones_file, \
     ...
     open("Not_launched_videos.txt", "w") as not_launched, \
    ...
    while True:
        ...
        max_line = max(delimitation_lines['x1'])
        for index, line in delimitation_lines.iterrows():    
            cv.line(frame, (line['x1'], line['y1']), (line['x2'], line['y2']), (0, 255, 0), 2)
            cv.putText(frame, f'Wall {index+1}', (max_line+50, line['y1']), cv.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 5)
