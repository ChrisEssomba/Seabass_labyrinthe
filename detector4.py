...
#video = cv.VideoCapture("D:/FutureExpertData/Computervision/cortado_laberinto3.mp4", cv.CAP_FFMPEG)
...
with open("zone_transitions.txt", "w") as zones_file, \
    ...
    ## Add in a file named lines_positions.txt  for each frame the position of the 4 coordinates of the lines ##
        ...
        for index, row in df_sorted.iterrows():
            ...
            if np.abs(row['y1'] - yy) > 100 and len(areas) <4:
                yy = row['y1']
                ...
                try:
                    ...
                    if y_botella > height / 2:
                        if center_y > 0 and center_y <= areas[0]:
                            current_zone = 1
                        elif (center_y > areas[0] and center_y <= areas[1] ) and xmax < x_tank:
                            current_zone = 2
                        elif (center_y > areas[1]   and center_y <= areas[2]) and xmax > x_tank:
                            current_zone = 3
                        elif (center_y > areas[2]  and center_y <= areas[3]) and xmax < x_tank:
                            current_zone = 4
                        elif (center_y > areas[3]) and xmax  > x_tank:
                            current_zone = 5
                    else:
                        if center_y < height and center_y >= areas[len(areas) - 1]:
                            current_zone = 1
                        elif (center_y < areas[len(areas) - 1]  and center_y >= areas[len(areas) - 2]) and xmax  > x_tank:
                            current_zone = 2
                        elif (center_y < areas[len(areas) - 2]  and center_y >= areas[len(areas) - 3])and xmax < x_tank:
                            current_zone = 3
                        elif (center_y < areas[len(areas) - 3]  and center_y >= areas[len(areas) - 4]) and xmax > x_tank:
                            current_zone = 4
                        elif (center_y < areas[len(areas) - 4]) and xmax  < x_tank:
                            current_zone = 5
    # Initialize or update the detection counter for the fish in the current zone
                    ...
                    if zone_detection_counter[fish_id][current_zone] >= threshold and current_zone !=0:
                        #I check if it's not in the file and it's the consecutive number of the last one (or just 1)
                        if (current_zone not in zones_written) and (current_zone==1 or current_zone == list(zones_written.keys())[-1]+1):
                            ...
                            zones_file.write(f"Frame {(index_frame - threshold) /fps}s: Fish {fish_id} transitioned to Zone {current_zone}, {zone_detection_counter[fish_id][current_zone]} \n")
