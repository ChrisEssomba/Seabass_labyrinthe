...
video = cv.VideoCapture("L:\ENVIROBASS_NOE\TESTS\APRENDIZAJE\S_26_29_MAYO\S_26_29_D2\S_26_29_D2_B5\GX026356.MP4")
...
#We add a new file to return the more information about the bounding box
with open("zone_transitions.txt", "w") as zones_file, \
     ...
     open("bb_positions.txt", "w") as bb_file, \
     open("box_positions.txt", "w") as box_file, \
                ...
                try:
                    ...
                    # Because the endpoint (bottle of food) is at the bottom of the tank we start counting from the very first pixel (0)
                            if y_center > 0 and y_center <= line1:
                                current_zone = 1
                            elif ((y_center > line1 and y_center <= line2) and xmax < x_middle_tank) or (y_center>(line1+line2)/2 and y_center<=line2):
                                current_zone = 2
                            elif ((y_center > line2 and y_center <= line3) and xmin > x_middle_tank) or (y_center>(line2+line3)/2 and y_center<=line3):
                                current_zone = 3
                            elif ((y_center > line3 and y_center <= line4) and xmax < x_middle_tank) or(y_center>(line3+line4)/2 and y_center<=line4):
                                current_zone = 4
                            elif ((y_center > line4) and xmin > x_middle_tank) or (y_center > line4 + (line4-line3)/2):
                                current_zone = 5
                            else:
                                current_zone=0
                    ...
                    cv.putText(frame, f'ID: {idx}, zone: {current_zone}, conf : {confidence}', (int(xmin), int(ymin) - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                    ...
                    if index_frame<=fps and confidence>=0.80:
                    ...
                    if dic_occ[idx] >= threshold and current_zone != 0:
                        ...
                        if (current_zone not in zones_written and current_zone==2) or (current_zone not in zones_written and current_zone == list(zones_written.keys())[-1]+1):
